...
function nuevaTab(){
  ...
  $('#lista_tabs').find(`.active`).removeClass(`active`);
  $(`#divs_tabs`).find(`.active`).removeClass(`active`);
  $('#lista_tabs').append(`<li style="white-space: nowrap;" id="li_entrada_${openTabs}" role="presentation" class="active">
      <a href="#entrada_${openTabs}" id="a_entrada_${openTabs}" style="padding-right: 5px !important; white-space: nowrap;" aria-controls="entrada_${openTabs}" data-toggle="tab" role="tab">Entrada ${openTabs} <span style="margin-left: 10px !important; display: inline-block;" class="close">×</span> </a>
    </li>`);
  $('#divs_tabs').append(`<div role="tabpanel" class="tab-pane active" id="entrada_${openTabs}">
      ...
      class="form-control" placeholder="Entrada ${openTabs}" spellcheck="false" required></textarea>
    </div>`);
  $(`#a_entrada_${openTabs}`).trigger('click');
}
...
function ejectuarAnalisis2(){
    ...
    var texto = $('#divs_tabs').find(".active").find("textarea").val();
